...
//        llama_backend_free()
    ...
    deinit {
        energyEvents["model_free.start"] = String(format: "%.0f", Date().timeIntervalSince1970 * 1_000_000_000)
        self.save_state()
        print("deinit LLaMa")
        self.destroy_objects()
        print("LLaMa deinited")
        energyEvents["model_free.end"] = String(format: "%.0f", Date().timeIntervalSince1970 * 1_000_000_000)
    }
